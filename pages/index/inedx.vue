					...
					<view class="page-section-spacing">
						<swiper class="myswiper" circular="true" indicator-dots="true" autoplay="true" interval="3500"
							...
							<swiper-item class="swiper-list" v-for="(item, index) in bannerList" :key="index">
								<view class="swiper-item uni-bg-red" @click="article(index)">
	...
	export default {
		...
		methods: {
			article(index){
				if(index%2==0){
					uni.navigateTo({
					    url: './article?index='+'https://mp.weixin.qq.com/s/B14b7MjOaBwKA_XDvYOWFQ'
					});
				}else{
					uni.navigateTo({
					    url: './article?index='+'https://mp.weixin.qq.com/s/JJuAfLvXSg2-bxtILbzNJA'
					});
				}
